...
def card_ranks(hand):
    ...
    ranks = sorted(["__23456789TJQKA".index(c) for c, s in hand], reverse=True)
    if ranks == [14, 5, 4, 3, 2]:
        ranks = [5, 4, 3, 2, 1]
    return ranks
...
def two_pair(ranks):
    high_card = kind(ranks, 2)
    low_card = kind(sorted(ranks), 2)
...
def hand_rank(hand):
    ...
    return (
        ...
        (2, two_pair(ranks), ranks) if two_pair(ranks) else
